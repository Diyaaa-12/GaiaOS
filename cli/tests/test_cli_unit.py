...
def test_cli_version() -> None:
    ...
    assert "gaiaos-cli version:" in result.stdout
    assert "gaiaos-sdk version:" in result.stdout
...
def test_auth_login(mock_save_config: MagicMock, mock_get_sdk_client: MagicMock) -> None:
    ...
    assert result.exit_code == 0
    assert "Successfully authenticated as 'testuser@gaiaos.io'" in result.stdout
...
def test_auth_status(mock_load_config: MagicMock) -> None:
    ...
    assert result.exit_code == 0
    assert "Logged in as operator" in result.stdout
...
def test_investigate_create(mock_get_sdk_client: MagicMock) -> None:
    ...
    assert result.exit_code == 0
    assert "Investigation submitted successfully" in result.stdout
...
def test_investigate_get(mock_get_sdk_client: MagicMock) -> None:
    ...
    assert result.exit_code == 0
    assert "inv-9999" in result.stdout
...
def test_investigate_trace(mock_get_sdk_client: MagicMock) -> None:
    ...
    assert result.exit_code == 0
    assert "Execution Trace Graph" in result.stdout
...
def test_admin_api_keys_create(mock_get_sdk_client: MagicMock) -> None:
    ...
    assert result.exit_code == 0
    assert "API Key 'CI Test Key' created successfully" in result.stdout
    assert "gaia_secret_key_123" in result.stdout
...
def test_admin_api_keys_list(mock_get_sdk_client: MagicMock) -> None:
    ...
    assert result.exit_code == 0
    assert "Active API Keys" in result.stdout
...
def test_admin_api_keys_revoke(mock_get_sdk_client: MagicMock) -> None:
    ...
    assert result.exit_code == 0
    assert "API Key 'k-1' successfully revoked" in result.stdout
